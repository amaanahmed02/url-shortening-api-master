...
export default function Urlbox() {
  ...
  const [link, setLink] = useState([
    {
      originalLink: "https://github.com/amaanahmed02",
      shortenLink: "https://9qr.de/WcMfuy",
    },
  ]);
  ...
  async function getLink(longUrl) {
    try {
      axios
        ...
        .then((responseData) => {
          console.log(
            `${responseData.data.result.full_short_link2} from response`
          );
    ...
    <div className="urlbox-container">
      <div className="url-form">
        <form action="#" method="post" onSubmit={handleSubmit}>
          ...
          />
          <button
            className={loading ? "cta rounded-sm loading" : "cta rounded-sm"}
            type="submit">
          ...
          <Tween key={i} from={{ opacity: 0 }} ease="elastic" duration={5}>
            <div className="url-container">
              ...
              <CopyToClipboard text={eachLink.shortenLink}>
                <button className="rounded-sm cta" onClick={handleCopy}>
                  Copy
                </button>
              </CopyToClipboard>
            </div>
